...
async function installPhpNode() {
  ...
  if (!metaResponse.ok)
    throw new Error(
      `Impossible de joindre le registre NPM: ${metaResponse.statusText}`
    );
  ...
  if (!fs.existsSync(TEMP_META_DIR))
    fs.mkdirSync(TEMP_META_DIR, {recursive: true});
  ...
  if (!response.ok)
    throw new Error(
      `Erreur HTTP lors du téléchargement: ${response.statusText}`
    );
  ...
  const pkg = JSON.parse(
    fs.readFileSync(path.join(TEMP_META_DIR, "package.json"), "utf-8")
  );
  ...
  if (deps) {
    ...
    try {
      execSync(`npm install ${deps} --no-save --no-package-lock`, {
        stdio: "inherit",
      });
    } catch (e) {
