...
async function installPhpNode() {
  ...
  console.log(`🚀 Installation de PHP v${version} terminée avec succès !`);
    ...
    // --- ÉTAPE 6 : EXTRACTION VENDOR
  if (fs.existsSync('vendor.tar.gz')) {
      console.log('📦 Extraction de vendor.tar.gz...');
      await x({ file: 'vendor.tar.gz', cwd: './' });
  }
  console.log(`🚀 Installation des dépendances Laravel terminée avec succès !`);


  // --- ÉTAPE 7 :  Création automatique du .env
  if ( fs.existsSync('.env.example')) {
      console.log('⚙️  Création du fichier .env...');
