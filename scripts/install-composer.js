...
const destinationDirectory = process.env.INIT_CWD ? `${process.cwd()}/${env.composer.path}` : env.composer.path;
const destinationFilePath = `${destinationDirectory}/${env.composer.name}`;

function ensureDestinationDirectory() {
  fs.existsSync(destinationDirectory) || fs.mkdirSync(destinationDirectory, { recursive: true });
}
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (contentLength && Number(contentLength) > 5 * 1024 * 1024) {
        console.warn(`Proxy response larger than 5MB (Content-Length=${contentLength}), falling back to direct download.`);
      } else {
        const arrayBuffer = await resp.arrayBuffer();
        if (arrayBuffer.byteLength > 5 * 1024 * 1024) {
          console.warn(`Proxy returned >5MB of data (${arrayBuffer.byteLength} bytes), falling back to direct download.`);
        } else {
          const buffer = Buffer.from(arrayBuffer);
          fs.writeFileSync(destinationFilePath, buffer, { mode: 0o755 });
          console.log(`Downloaded composer to ${destinationFilePath} via CodeTabs proxy.`);
          return;
        }
      }
    } else {
      console.warn(`Proxy request failed with status ${resp.status}, falling back to direct download.`);
    }
  } catch (err) {
    console.warn(`Proxy fetch failed: ${err?.message ?? err}. Falling back to direct download.`);
  }
  ...
  try {
    await new Promise((resolve, reject) => {
      const req = https.get(composerUrl, (res) => {
        ...
        const fileStream = fs.createWriteStream(destinationFilePath, { mode: 0o755 });
        res.pipe(fileStream);
    ...
    console.log(`Downloaded composer to ${destinationFilePath} via direct https.get.`);
  } catch (err) {
    throw err;
  }
})();